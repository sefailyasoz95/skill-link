...
export default function MessagesClient({ userId, initialProfile }: { userId: string; initialProfile: Profile | null }) {
	...
	useEffect(() => {
		...
		const fetchProfileAndMessages = async () => {
			try {
				// Only fetch profile if not provided from server
				if (!profile) {
	...
	}, [user, userId, router, toast, profile]);
