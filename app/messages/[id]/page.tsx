	...
	useEffect(() => {
		...
		const fetchChatData = async () => {
          ...
          `
				...
				const isUserMember = chatMembers.some((member: any) => member.user_id === user.id);
            ...
            `
					...
					if (!skillsError && userSkills) {
						const skills = userSkills.map((us: any) => us.skill).filter(Boolean);
			...
			.on(
				...
				async (payload) => {
					...
					const { data: senderData } = await supabase
						.from("users")
						.select("id, full_name, username, profile_picture, bio, location, availability, created_at")
		...
		return (
			<div className='flex flex-col space-y-3 p-1'>
				<div className='flex items-center space-x-3'>
					...
					<div>
						<h4 className='font-semibold text-base'>{userInfo.full_name || userInfo.username || "User"}</h4>
				...
				{userInfo.bio && <p className='text-sm text-muted-foreground'>{userInfo.bio}</p>}
				...
				<div className='mx-auto max-w-5xl'>
					<Card>
						<CardHeader>
							...
							<CardDescription>There was a problem loading this conversation</CardDescription>
						</CardHeader>
						<CardContent>
							...
							<div className='space-y-4'>
								{[1, 2, 3, 4, 5].map((i) => (
									<div key={i} className={`flex ${i % 2 === 0 ? "justify-end" : "justify-start"}`}>
										...
										<div>
											<Skeleton className={`h-[60px] ${i % 2 === 0 ? "w-[250px]" : "w-[200px]"} rounded-lg`} />
						...
						<div className='flex items-center space-x-3'>
							...
							<div className='flex flex-col'>
								...
								<CardDescription className='mt-1'>
									{chat?.is_group ? `${participants.length} participants` : "Private conversation"}
								</CardDescription>
							</div>
								...
								{messages.map((message) => {
									...
									return (
										<div key={message.id} className={`flex ${isCurrentUser ? "justify-end" : "justify-start"}`}>
														...
														<div className='h-10 w-10 rounded-full overflow-hidden bg-primary/10 flex-shrink-0 mr-2 cursor-pointer'>
															{senderInfo.profile_picture ? (
																...
																<div className='h-full w-full flex items-center justify-center text-sm font-medium text-primary'>
																	{(senderInfo.full_name || senderInfo.username || "U").charAt(0).toUpperCase()}
																</div>
															)}
														</div>
													</HoverCardTrigger>
											...
											<div className={isCurrentUser ? "max-w-[70%]" : "max-w-[65%]"}>
												<div
