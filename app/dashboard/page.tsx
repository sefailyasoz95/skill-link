...
import type { User, Skill, CollabNeed, ProfileView, Connection, ConnectionStatus } from "@/lib/types";
				...
				? (skillsData as any[]).reduce((acc: Skill[], item: any) => {
						if (item?.skills) {
							...
							if (Array.isArray(item.skills)) {
								...
								if (item.skills.length > 0 && typeof item.skills[0] === "object") {
								...
								<CardContent>
									{loadingData ? (
										...
										<div className='space-y-4'>
											...
											{(!profile.full_name || userSkills.length === 0 || collabNeeds.length === 0) && (
												...
												<Card className='overflow-hidden transition-all hover:shadow-md'>
													<CardContent className='p-4'>
														<div className='flex items-start space-x-4'>
															...
															<div className='flex-1 space-y-1'>
																<p className='font-medium'>{connection.requesting_user?.full_name || "Unnamed User"}</p>
																<p className='text-xs text-muted-foreground'>
