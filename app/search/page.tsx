...
export default function SearchPage() {
    ...
    const fetchProfiles = async () => {
      try {
        ...
        data?.forEach(profile => {
          ...
          profile.collaboration_needs?.forEach(need => needs.add(need));
          profile.collaboration_terms?.forEach(term => terms.add(term));
        });
  ...
  useEffect(() => {
    ...
    if (filters.needs.length) {
      result = result.filter(profile => 
        profile.collaboration_needs?.some(need => filters.needs.includes(need))
      );
    }

    if (filters.terms.length) {
      result = result.filter(profile => 
        profile.collaboration_terms?.some(term => filters.terms.includes(term))
      );
    }
                ...
                {filteredProfiles.map((profile) => (
                  <Card key={profile.id}>
                    <CardContent className="p-4 sm:p-6">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">{profile.availability || 'Availability not specified'}</p>
                          </div>
                        </div>
                      ...
                      {profile.bio && (
                        <p className="mt-4 text-sm line-clamp-2">{profile.bio}</p>
                      )}
