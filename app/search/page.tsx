...
export default function SearchPage() {
	...
	useEffect(() => {
			...
			result = result.filter(
				(profile) =>
					...
					profile.collab_needs?.some((need) =>
						need.conditions?.some((condition) => condition.toLowerCase().includes(query))
					)
			);
		}
		...
		if (activeFilters.skills.length) {
			result = result.filter((profile) => profile.skills?.some((skill) => activeFilters.skills.includes(skill.name)));
		}
		...
		if (activeFilters.needs.length) {
			result = result.filter((profile) =>
				profile.collab_needs?.some((need) =>
					...
					need.conditions?.some((condition) => activeFilters.needs.includes(condition))
				)
			);
		}
		...
		if (activeFilters.terms.length) {
			result = result.filter((profile) =>
				profile.collab_needs?.some((need) =>
					...
					need.conditions?.some((condition) => activeFilters.terms.includes(condition))
				)
			);
		}
									...
									<Card key={profile.id}>
										...
										<CardContent className='space-y-4'>
											...
											{profile.collab_needs && profile.collab_needs.length > 0 && (
												<div>
													<h4 className='text-sm font-medium'>Collaboration Terms</h4>
													<div className='mt-1 flex flex-wrap gap-1'>
														...
														{profile.collab_needs && profile.collab_needs.length > 3 && (
															<Badge variant='outline'>+{profile.collab_needs.length - 2}</Badge>
														)}
													</div>
												</div>
											)}
