...
const profileFormSchema = z.object({
  ...
  past_projects: z.array(
    z.object({
      ...
      url: z.string().url('Must be a valid URL').or(z.string().length(0)).nullable().transform(val => val === '' ? null : val),
    })
  ).nullable(),
});
...
const skillOptions = [
  ...
  'Marketing', 'Content Writing', 'SEO', 'Social Media', 'Business Strategy',
  'Product Management', 'Data Analysis', 'AI/ML', 'DevOps', 'Sales',
  'Customer Support', 'Video Editing', 'Audio Production', 'Photography',
  'Illustration', 'Animation', '3D Modeling', 'Game Development',
  'Blockchain', 'Copywriting', 'Email Marketing',
];

const collaborationTermOptions = [
  'Revenue Share', 'Equity', 'Fixed Fee', 'Hourly Rate',
  'Skill Exchange', 'Co-founder', 'Advisor', 'Mentorship',
];
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (data) {
          ...
          if (data.skills) {
            const custom = data.skills.filter(skill => !skillOptions.includes(skill));
            setCustomSkills(custom);
          }
          
          if (data.collaboration_needs) {
            const custom = data.collaboration_needs.filter(need => !skillOptions.includes(need));
            setCustomNeeds(custom);
          }
          
          if (data.collaboration_terms) {
            const custom = data.collaboration_terms.filter(term => !collaborationTermOptions.includes(term));
                        ...
                        <FormControl>
                          <div className="flex items-center space-x-4">
                            ...
                            <div className="text-sm text-muted-foreground">
                              Drop an image here or click to upload a profile picture.<br />
