		...
		const fetchProfile = async () => {
			try {
				...
				if (data) {
					...
					if (data.skills) {
						const custom = data.skills.filter((skill: any) => !skillOptions.includes(skill));
						setCustomSkills(custom);
					}

					if (data.collaboration_needs) {
						const custom = data.collaboration_needs.filter((need: any) => !skillOptions.includes(need));
						setCustomNeeds(custom);
					}

					if (data.collaboration_terms) {
						const custom = data.collaboration_terms.filter((term: any) => !collaborationTermOptions.includes(term));
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												...
												<FormDescription>Select the collaboration terms you're open to</FormDescription>
												<FormMessage />
											</FormItem>
										)}
									/>
